...
exports.smsg = (conn, m, store) => {
    ...
    let nua = 0
    conn.reply = async (teks) => {
        await turu(5)
        if (nua < 4) {
            nua = 999
            return await conn.sendFakeLink(m.chat, teks, salam, ownername, logo, myweb, pushname, m)
        } else {
    ...
    conn.sendPesan = async (...args) => {
        await turu(5)
        await conn.sendMessage(...args)
    }
    return m
}
exports.delay = (tod) => {
